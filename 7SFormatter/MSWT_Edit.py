...
def remove_tagged_lines(text):
    patterns = [r'^\[i\]$', r'^\[\/i\]$']  # Corrected patterns
    lines = text.split("\n")
    cleaned_lines = []
    for line in lines:
...
def remove_lines(text):
    ...
    for line in lines:
        ...
        if line.strip() == '' or line.strip().startswith("[b]Page"):
            cleaned_lines.append(line)
        # Check if the line matches the pattern with [b][/b] tags and also matches the other deletion patterns
        elif re.match(r'\[b\].*\[\/b\]', line.strip()) and (re.match(r'^[0-9]+\s+[a-zA-Z]+.*$', line.strip()[3:-4]) or re.match(r'^[0-9]+\.\d+', line.strip()[3:-4])):
            cleaned_lines.append('')
        # Check for lines that don't match the specified patterns
        elif not re.match(r'^\[i\]$|^<\/i>$', line.strip()) and not re.match(r'^[0-9]+\.[0-9]+|^[0-9]+', line.strip()) and not re.match(r'^[0-9]+\s+[a-zA-Z]+$', line.strip()):
            cleaned_lines.append(line)
    cleaned_text = '\n'.join(cleaned_lines)
    return cleaned_text
...
def main(docx_file):
    ...
    with open(output_file, "w", encoding="utf-8") as file:
        ...
        cleaned_text = re.sub(r'(\[/[bik]\]) (\[/[bik]\])', r'\1', cleaned_text)
